...
export default function FAQ({ content }: { content: FAQContent }) {
  ...
  return (
    <section id="faq" className="py-14 md:py-24 bg-neutral-50 dark:bg-primary-dark transition-colors duration-300">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 md:mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-bold font-serif text-primary dark:text-white mb-6">{content.heading}</h2>
        ...
        <div className="space-y-4">
          {content.items.map((item, index) => (
            <div key={index} className={`group overflow-hidden rounded-xl border transition-all duration-300 ${expandedIndex === index ? 'bg-white dark:bg-gray-800 border-accent shadow-md' : 'bg-white dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
              <button onClick={() => setExpandedIndex(expandedIndex === index ? null : index)} className="w-full px-4 py-4 md:px-6 md:py-5 flex items-center justify-between text-left focus:outline-none">
                <h3 className={`text-base md:text-lg font-bold font-serif transition-colors duration-200 text-primary dark:text-white`}>
                  {item.question}
                </h3>
